...
def trainARImage(train_dataset_path, val_dataset_path, verbose=False, data_npy_exists = False):
    ...
    for epoch in range(numEpochs):
        ...
        for i, batch in enumerate(data_loader):
            ...
            batchReplicate = batch[0]
            batchReplicate = torch.reshape(batchReplicate, (batch[0].shape[0], batch[0].shape[2], batch[0].shape[3]))
            trainPrediction = model(batch[0])
            trainPrediction = torch.reshape(trainPrediction, (batch[0].shape[0], batch[0].shape[2], batch[0].shape[3]))
            loss = criterion(batchReplicate, trainPrediction)
            loss = torch.sum(loss)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
          model.eval()
          valPredTensor = model(val_imgs)
          loss = criterion(val_imgs, valPredTensor)
          val_loss = torch.sum(loss)

        # show the plots
        if epoch != 0:
            prevtrain_loss = prevtrain_loss.cpu()
            train_loss = train_loss.cpu()
            prevval_loss = prevval_loss.cpu()
