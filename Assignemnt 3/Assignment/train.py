...
def NTcrossentropy(vtx_feature, pts_feature, corr, tau=0.07):      
    ...
    num = torch.mul(corr_vtx_feature, corr_pts_feature)
    num = torch.sum(num, 1)
    num = torch.div(num, tau)
    num = torch.exp(num)

    den = torch.mm(corr_vtx_feature, torch.transpose(corr_pts_feature, 0, 1))
    den = torch.div(den, tau)
    den = torch.exp(den)
    den = torch.sum(den, 1)
    ...
    loss = torch.div(torch.sum(loss), list(loss.size())[0])    
    return loss
...
def fit_rotation(vtx, pts, vtx_feature, pts_feature, corrmask):
    ...
    # Steps to retrieve coordinates
    similarityMatrix = torch.matmul(vtx_feature, pts_feature.transpose(0, 1))[corrmask>0.5]
    
    # Remove indices with lower confidence & Select that are greater than 0.5
    _, idx = torch.max(similarityMatrix, dim=1)
    
    vtx_pts = vtx[corrmask > 0.5]
    similar_pts = pts[idx]
    
    # Calculate centroids     
    centroidP = torch.mean(vtx_pts,0)
    centroidQ = torch.mean(similar_pts,0)
    
    # Subtracte centroid vectors
    P = vtx_pts - centroidP
    Q = similar_pts - centroidQ
    
    # Compute Scaling Factor
    scale = torch.pow(torch.norm(P, p = 2)/torch.norm(Q, p = 2), 1/4)
    centroidP_ = centroidP/scale
    centroidQ_ = centroidQ/scale
    
    P = vtx_pts - centroidP_
    Q = similar_pts - centroidQ_
    
    # Optimal Translation applied
    S = torch.matmul(torch.transpose(P, 0, 1), Q)
    
    # Optimal Rotation    
    U, D, Vt = torch.svd(S)
    RotationMat = torch.matmul(U, torch.transpose(Vt, 0, 1))
    
    # Reflection handling
    if (torch.linalg.det(RotationMat)) < 0:
      Vt[2] = Vt[2]*-1
    
    R = Rotation.from_matrix(RotationMat.cpu().numpy())
...
# paths you may want to adjust, but it's better to keep the defaults    
    ...
    # train_corrmask = 1 model_corr_best.pth.tar
 ...
 # various options for testing and evaluation
    ...
    parser.add_argument("--distance_threshold", default=0.04, type=float, 
